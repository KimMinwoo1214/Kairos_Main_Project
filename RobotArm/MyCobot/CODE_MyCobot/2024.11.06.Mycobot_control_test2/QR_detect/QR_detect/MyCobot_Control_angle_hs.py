...
# 전역 변수로 카메라 객체 설정
cap = None
qr_code_detected_event = threading.Event()
...
def detect_qr_code(frame):
    ...
    for obj in decoded_objects:
        ...
        qr_code_detected_event.set()  # QR 코드가 감지되면 이벤트 설정
...
class MyCobotController(QWidget):
    ...
    def execute_scenario(self):
        ...
        qr_code_detected_event.clear()  # 감지 이벤트 초기화
        if qr_code_detected_event.wait(timeout=10):  # QR 코드 감지 대기
            print("QR Code detected at the first position.")
        else:
            print("QR Code not detected within timeout.")
