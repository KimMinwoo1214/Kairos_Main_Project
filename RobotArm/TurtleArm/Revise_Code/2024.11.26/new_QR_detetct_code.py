...
while True:
    ...
    decoded_text, points, _ = qr_detector.detectAndDecode(undistorted_frame)
    if points is not None:  # If a QR code is detected
        ...
        if decoded_text:
            cv2.putText(undistorted_frame, decoded_text, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            print(f"Detected QR Code: {decoded_text}")

    # Display the frame with the QR code highlighted
    cv2.imshow("Undistorted Frame with QR Code", undistorted_frame)
