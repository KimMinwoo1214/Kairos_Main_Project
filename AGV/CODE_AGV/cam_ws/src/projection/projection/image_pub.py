...
class UdpVideoYOLONode(Node):
    def __init__(self):
        ...
        self.image_pub = self.create_publisher(Image, '/camera_image', 10)  # 원본 이미지 퍼블리셔

        # CV Bridge for image conversion
        self.bridge = CvBridge()

        # UDP Video stream setup
        self.BUFF_SIZE = 65536
        self.client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.client_socket.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, self.BUFF_SIZE)
        
        # Server details
        self.host_ip = '192.168.0.137'  # Video server IP address
        self.port = 9090
        ...
        # Load YOLOv8 model
        self.model = YOLO('/home/kmw/best.pt')  # Replace with the actual path to your YOLO model
        self.run_client()
    ...
    def run_client(self):
        while rclpy.ok():
            try:
                ...
                # Decode the packet into an image frame
                try:
